...
<div class="eXtremeTable" >
		...
		<td>			
			<select style="width:120px" name="dept.deptId">
				...
				<c:forEach items="${deptList }" var="d">
					<option value="${d.deptId}"<c:if test="${d.deptId==userMsg.dept.deptId}">selected="selcted"</c:if>>${d.deptName} </option>
				</c:forEach>			
			</select>		
		</td>
		<td>真实姓名：</td>
		...
		<td>			
			<select style="width:120px" name="userInfo.manager.userInfoId">
				...
				<c:forEach items="${infoList }" var="p">
					<option value="${p.userInfoId}" <c:if test="${p.userInfoId==info.manager.userInfoId }">selected="selected"</c:if>
					>${p.name} </option>
				</c:forEach>			
			</select>
	...
	<tr class="odd">
		...
		<td>
			...
			<select style="width:120px" name="userInfo.userLevel">
				...
				<option value="2" <c:if test="${info.userLevel==2 }">selected="selected"</c:if>>副总经理</option>	
	...
	<tr class="odd">
		<td>备注信息：</td>
		<td colspan="3">
			<textarea name="userInfo.remark" style="height:80px;width:100%" value="${info.remark }"></textarea>		
		</td>
